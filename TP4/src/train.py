...
def main() -> None:
    ...
    # NOTE pratique:
    # - Pour MLP/GCN full-batch: on peut mettre tout sur GPU
    # - Pour NeighborLoader (sage): le sampling est souvent plus stable sur CPU
    #   puis on envoie chaque batch sur GPU.
    if args.model == "sage":
        pyg_data = data.pyg_data  # reste sur CPU pour NeighborLoader
    else:
    ...
    if args.model == "sage":
        ...
        print("sampling.batch_size:", bs)
        print("sampling.num_neighbors:", [n1, n2])
    ...
    for epoch in range(1, epochs + 1):
        ...
        if args.model in ["mlp", "gcn"]:
            with Timer() as t:
                ...
                loss = criterion(logits[train_mask_dev], y_dev[train_mask_dev])
        ...
        model.eval()
        with torch.no_grad():
            # évaluation full-batch sur tout Cora (sur device)
            x_eval = data.x.to(device)
            y_eval = data.y.to(device)
            edge_index_eval = data.edge_index.to(device)
            train_mask_eval = data.train_mask.to(device)
            val_mask_eval = data.val_mask.to(device)
            test_mask_eval = data.test_mask.to(device)

            if args.model == "mlp":
                logits = model(x_eval)
            else:
                logits = model(x_eval, edge_index_eval)

            m_train = compute_metrics(logits[train_mask_eval], y_eval[train_mask_eval], data.num_classes)
            m_val = compute_metrics(logits[val_mask_eval], y_eval[val_mask_eval], data.num_classes)
            m_test = compute_metrics(logits[test_mask_eval], y_eval[test_mask_eval], data.num_classes)
