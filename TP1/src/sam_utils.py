...
def predict_mask_from_box(
    ...
    if x2 <= x1 or y2 <= y1:
        raise ValueError(f"BBox invalide après clamp: {(x1, y1, x2, y2)}")
...
def predict_masks_from_box_and_points(
    ...
    Retourne (masks, scores) où :
      - masks : (K, H, W) bool
      - scores : (K,) float
    Format attendu par SAM :
      - point_coords : float32 (N,2) en pixels (x,y)
      - point_labels : int64 (N,) avec 1=FG, 0=BG
    """
    predictor.set_image(image_rgb)
