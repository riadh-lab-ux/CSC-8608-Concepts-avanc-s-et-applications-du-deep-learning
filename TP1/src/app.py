...
CKPT_PATH = "TP1/models/sam_vit_h_4b8939.pth"   # à adapter
MODEL_TYPE = "vit_h"
...
# ---- Prévisualisation bbox + points ----
def draw_preview(image_rgb: np.ndarray, box_xyxy: np.ndarray, points):
    preview = image_rgb.copy()
    bgr = cv2.cvtColor(preview, cv2.COLOR_RGB2BGR)

    x1p, y1p, x2p, y2p = [int(v) for v in box_xyxy.tolist()]
    cv2.rectangle(bgr, (x1p, y1p), (x2p, y2p), color=(0, 255, 0), thickness=2)
...
# 5) Sélection du masque candidat + affichage
