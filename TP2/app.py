...
prompt = st.text_area(
    "Prompt",
    value="ultra-realistic e-commerce product photo of a backpack on a white background, studio lighting, soft shadow, very sharp",
)
negative_prompt = st.text_area(
    "Negative prompt",
    value="text, watermark, logo, low quality, blurry, deformed",
)
...
if run:
    ...
    st.image(
        img,
        caption=f"{config['mode']} | {config['scheduler']} | seed={config['seed']}",
