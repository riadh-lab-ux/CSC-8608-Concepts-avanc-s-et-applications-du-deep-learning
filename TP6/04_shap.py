...
model.fit(X_train, y_train)  # TODO rempli

print(f"Accuracy du Random Forest : {model.score(X_test, y_test):.4f}")

# 3. Explicabilité Post-Hoc avec SHAP
explainer = shap.TreeExplainer(model)  # TODO rempli

# Calcul SHAP sur le jeu de test
shap_values = explainer(X_test)  # TODO rempli (Explanation object)

# SHAP classe 1 (Bénigne)
...
plt.savefig(output_local, dpi=200)
plt.close()
...
plt.savefig(output_global, dpi=200)
plt.close()
